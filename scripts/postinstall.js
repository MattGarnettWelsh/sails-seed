...
fs.exists(homeDir + "node_modules/sails", function (exists) {
	if (exists) {
		var sailsVersion = require(requireDir +
			"node_modules/sails/package.json").version;
		createSeed(function () {
			if (semver.lt(sailsVersion, "0.11.0")) createHook(function () {});
		});
	} else {
		util.debug(
			"You are not using a sails project. Please use a sails project."
		);
	}
});
	...
	fs.exists(homeDir + "api/hooks/seed/index.js", function (exists) {
		if (exists) {
			...
			fs.readFile("./templates/userHook.js", function (err, data) {
				fs.writeFile(
					homeDir + "api/hooks/seed/index.js",
					data,
					function (err) {
